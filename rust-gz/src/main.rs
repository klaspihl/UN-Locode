...
fn parse_csv_gz(path: &str) -> Vec<Vec<String>> {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let args = parse_args();
    let data_dir = "./data";
    let code_list_path = format!("{}/code-list.csv.gz", data_dir);
    let country_codes_path = format!("{}/country-codes.csv.gz", data_dir);

    if args.name.is_none() && args.location.is_none() && args.country.is_none() {
        println!("Usage: set environment variables or arguments:\n  --name <location name>\n  --location <UN/LOCODE short name>\n  --country <country code or name>\n\nExample (environment):\n  docker run --rm --env location=yng --env country=se find-unlocation-gz\n\nExample (arguments):\n  docker run --rm find-unlocation-gz --location yng --country se");
        return Ok(());
    }

    let unlocode_rows = parse_csv_gz(&code_list_path);
    let country_rows = parse_csv_gz(&country_codes_path);
